...
## 파일 읽기 지침
- 모든 프로젝트 파일은 UTF-8 인코딩을 사용한다.
- 터미널 출력에서 한글이 깨져 보여도 콘솔 인코딩 문제로 간주하고, 해당 이유만으로 파일 전체를 재작성하지 마라.
기존 한글 및 비 ASCII 문자열은 명시적 수정 요청이 없는 한 그대로 유지하라.

## 문서 참조 Route
모든 Agent용 참조 문서는 .codex/ 디렉토리 하위에 존재한다. 다른 위치의 문서는 agent용 참조 문서가 아니다 읽지 말것.
